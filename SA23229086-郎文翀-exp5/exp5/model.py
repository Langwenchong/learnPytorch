...
class Transformer(nn.Module):
    ...
    def encode(self, src, src_mask):
        ...
        # 将嵌入后的序列和源掩码传递给encoder进行编码
        encoded_src = self.encoder(embedded_src, src_mask)
        return encoded_src
    ...
    def decode(self, memory, src_mask, tgt, tgt_mask):
        ...
        Args:
            memory (Tensor): 编码器的输出结果，即源序列的表示
            src_mask (Tensor): 源序列的掩码
            tgt (Tensor): 输入的目标序列
            tgt_mask (Tensor): 目标序列的掩码

        Returns:
            Tensor: 解码后的结果
        """
        # 使用tgt_embed对目标序列进行嵌入
        embedded_tgt = self.tgt_embed(tgt)
        # 将嵌入后的目标序列、编码器的输出结果以及掩码传递给decoder进行解码
        decoded_tgt = self.decoder(embedded_tgt, memory, src_mask, tgt_mask)
        return decoded_tgt
    ...
    def forward(self, src, tgt, src_mask, tgt_mask):
        ...
        decoded_output = self.decode(self.encode(src, src_mask), src_mask, tgt, tgt_mask)
...
def batch_greedy_decode(model, src, src_mask, max_len=64, start_symbol=2, end_symbol=3):
    ...
    # 初始化标志列表，用于判断每个样本是否已经完成预测
    stop_flag = [False for _ in range(batch_size)]
    # 初始化计数器
    count = 0

    # 使用编码器对输入序列进行编码，得到内部表示(memory)
    memory = model.encode(src, src_mask)
    # 初始化目标序列，每个样本以起始符号(start_symbol)作为起始
    tgt = torch.Tensor(batch_size, 1).fill_(start_symbol).type_as(src.data)

    # 循环生成序列，直到达到最大长度或者每个样本都生成了结束符号(end_symbol)
    for s in range(max_len):
        # 生成目标序列掩码，确保只能看到当前位置及之前的信息
        ...
        # 遍历每个样本，检查是否生成了结束符号，更新结果列表和标志列表
