...
log_name = f"{config.output_name}_graphs"
if not os.path.exists(log_name):
    os.mkdir(log_name)

writer = {
    'train_loss':SummaryWriter(f"./{log_name}/train_loss"),
    'val_loss': SummaryWriter(f"./{log_name}/val_loss"),
    'bleu_score':SummaryWriter(f"./{log_name}/bleu_score"),
}
...
def train(train_data, dev_data, model, model_par, criterion, optimizer):
    ...
    for epoch in range(1, config.epoch_num + 1):
        ...
        writer['train_loss'].add_scalar("train_loss/epoch",train_loss,epoch)
        # 模型验证
        model.eval()
        ...
        writer['val_loss'].add_scalar("val_loss/epoch",dev_loss,epoch)
        writer['bleu_score'].add_scalar("bleu_score/epoch",bleu_score,epoch)
...
class MultiGPULossCompute:
    ...
    def __init__(self, generator, criterion, devices, opt=None, chunk_size=5):
        # 将模型和损失函数发送到不同的GPU上
        self.generator = generator
        #  nn.parallel.replicate 的作用是 该函数用于将模型或模块复制到多个 GPU 上，返回一个模型或模块的列表
    ...
    def __call__(self, out, targets, normalize):
        total = 0.0
        # nn.parallel.scatter 的作用是该函数用于将输入数据分散到多个 GPU 上，返回一个包含分散数据的列表
