...
def train_word2vec_model(data_folder, algorithm='skipgram'):
    ...
    model = gensim.models.word2vec.Word2Vec(sentences=sentences, vector_size=200, workers=8, window=10, min_count=5,
                                            sg=sg)
...
def load_word2vec_embeddings(word2vec_file, word_map):
    ...
    for word in word_map:
        if word in w2v.key_to_index:
            embeddings[word_map[word]] = torch.from_numpy(w2v[word]).float()
...
预处理文本，包括将文本转为小写、规范换行符、去除无关字符。
返回处理后的文本字符串。
read_csv(csv_folder, split, sentence_limit, word_limit)：

读取包含原始训练数据的CSV文件，清理文档和标签，并进行词频统计。
返回文档列表、标签列表和词频统计。
create_input_files(csv_folder, output_folder, sentence_limit, word_limit, min_word_count=5, save_word2vec_data=True)：

用于创建用于训练模型的数据文件。
读取训练数据，保存用于训练word2vec的文本数据。
创建词映射（word map），将词映射到整数值，并保存为JSON文件。
编码和填充训练数据，然后保存为.pth.tar文件。
释放一些内存后，读取和预处理测试数据，对测试数据进行编码和填充，最后保存为.pth.tar文件。
train_word2vec_model(data_folder, algorithm='skipgram')：

用于训练word2vec模型以生成单词嵌入。
从word2vec训练数据中读取句子，初始化并训练word2vec模型。
将模型保存为.word2vec_model文件。
init_embedding(input_embedding)：

用于初始化嵌入张量，采用均匀分布的值。
load_word2vec_embeddings(word2vec_file, word_map)：

加载预训练的单词嵌入模型，并返回包含word map中单词嵌入的张量。
clip_gradient(optimizer, grad_clip)：

用于在反向传播期间剪切梯度，以防止梯度爆炸。
save_checkpoint(epoch, model, optimizer, word_map)：

保存模型的检查点，包括模型、优化器、词映射等信息。
AverageMeter类：
